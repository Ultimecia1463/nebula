...
const ChatItem = ({
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      Object.entries(socketQuery).forEach(([key, value]) => {
  ...
  const canEditMessage = useMemo(() => !deleted && isOwner && !fileUrl, [deleted, isOwner, fileUrl]);
...
export default React.memo(ChatItem, (prevProps, nextProps) => {
  return (
    ...
    prevProps.currentMember.id === nextProps.currentMember.id
  );
});
